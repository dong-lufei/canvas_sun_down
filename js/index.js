  ...
  init() {
    ...
    for (let i = 0; i < this.birdNum; i++) {
      const b = new Bird(
        ...
        Tool.randomNumber(0, -this.height / 2),
      )
      this.birds.push(b)
    }
    // ground
...
class Bird {
  ...
  draw() {
    ...
    ctx.quadraticCurveTo(
      this.x + this.r / 2,
      (Math.sin(this.a.rad) * this.r) / 3 + this.y - this.r / 2,
      this.x + this.r,
      (Math.sin(this.a.rad) * -this.r) / 10 + this.y,
    )
    ctx.stroke()
    ctx.beginPath()
    ctx.moveTo(this.x, this.y)
  ...
  updateParams() {
    ...
    if (this.ga < 0.2) {
      this.init(
        ...
        Tool.randomNumber(0, -canvas.height / 5),
      )
    }
  }
    ...
    if (canvas.carBehavior === 3) {
      ...
      if (e > 500) {
        ...
        if (this.x + this.r > canvas.width) {
          ...
          this.v.x *= -1
          this.v.y =
            Math.sin((Tool.randomNumber(270, 270) * Math.PI) / 180) * 20
        }
      }
    }
    if (this.y < 0) {
